...
const GuestQuiz = ({ id }) => {
    ...
    useEffect(async () => {
        try {
            setIsLoading(true);
            const rawRsponse = await fetch(`${getApiBaseUrl()}guest_quizes/${id}`, {
                method: 'GET',
                credentials: 'include'
            });
    ...
    const submitHandler = async (questionAnswers) => {
        ...
        const rawRsponse = await fetch(`${getApiBaseUrl()}submitGuestQuiz`, {
            method: 'POST',
            credentials: 'include',
            headers: {
    ...
    return (
        <div className="flex-container">
            <div style={{ padding: '20px' }} className="flex-container">
